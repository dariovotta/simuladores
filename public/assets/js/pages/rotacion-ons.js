...
function update() {
    ...
    const rows = calc.results.map((r, i) => `<tr>
        ...
        <td class="rate">${formatNumber(r.rate, 2)}% (x2)</td>
        ...
        <td class="t-left">${monthsLabel(r.months)}</td>
    </tr>`).join('');
    ...
    let html = `<div class="table-wrap"><table class="table table--numeric table--ranking">
        <thead><tr>
            ...
            <th>Der. Mercado</th><th>Costo Total</th><th class="t-left">Recupero</th>
        </tr></thead>
        <tbody>${rows}</tbody>
    </table></div>`;
    ...
    if (spread > 0) {
        ...
        html += `<div class="notice notice--warn"><strong>Atención:</strong> Las TIRs son iguales. La rotación solo genera costos sin ganancia adicional.</div>`;
    } else {
...
function renderChart(results) {
    ...
    const toneColor = { good: theme.pos, mid: theme.warn, bad: theme.neg };
